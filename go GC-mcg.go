...
// areas to minimize fragmentation while eliminating locks in the common case.说明是标记与清扫通过写屏障进行并行处理，GC是非代系也非紧凑性.内存分配是基于每个P、尺寸隔离分配的，最小化碎片，
// 与此同时减少了锁
//
// The algorithm decomposes into several steps. 算法分解几个步骤
...
//
// 1. GC performs sweep termination. GC操作扫描结束
//
//    a. Stop the world. This causes all Ps to reach a GC safe-point.停止这世界，引起所有的P达到GC安全点
//
//    b. Sweep any unswept spans. There will only be unswept spans if
//    this GC cycle was forced before the expected time.扫描未扫描的span。强制GC时候如果存在未被扫描的
//
// 2. GC performs the mark phase.GC标记阶段
//
...
//    状态从_GCoff ---> _GCMark, 启动写屏障，启动辅助线程，根节点入队。
//    直到所有P都扫描， 此时用STW
...
//    are immediately marked black.新分配的对象立马标记为黑色
//
...
//    操作root节点标记工作。扫描所有的栈，标记所有的全局全量。 resume 恢复协程
//    d. GC drains(扫描、外流、排泄) the work queue of grey objects, scanning each grey
...
//
// 5. When sufficient（足够，充足的） allocation has taken place, replay the sequence
...
func gcStart(trigger gcTrigger) {
	...
	gcBgMarkStartWorkers() // 启动后台mark线程

	systemstack(gcResetMarkState) // 设置pageMarks 为 0 ，heapArean结构体
	...
	systemstack(stopTheWorldWithSema) // 设置所有的P为PgcStop工作，并抢占g，preemtall->preemtone
	// Finish sweep before we start concurrent scan.
	systemstack(func() {
		finishsweep_m() // 开启新的纪元，待看
	})

	// clearpools before we start the GC. If we wait they memory will not be
	// reclaimed until the next GC cycle.
	clearpools() // central回收 sudosg，deferpool
	...
	// possible.
	setGCPhase(_GCmark) // 函数会修改表示当前GC阶段的全局变量和是否开启写屏障的全局变量
